...
class OrderShoesControllerTest < ActionDispatch::IntegrationTest
  ...
  test 'should create order_shoe' do
    assert_difference('OrderShoe.count') do
      post order_shoes_url,
           params: { order_shoe: { active: @order_shoe.active, discount: @order_shoe.discount, order_id: @order_shoe.order_id,
                                   price: @order_shoe.price, shoe_id: @order_shoe.shoe_id } }
    end
  ...
  test 'should update order_shoe' do
    patch order_shoe_url(@order_shoe),
          params: { order_shoe: { active: @order_shoe.active, discount: @order_shoe.discount, order_id: @order_shoe.order_id,
                                  price: @order_shoe.price, shoe_id: @order_shoe.shoe_id } }
